...
class PurchaseService:
    ...
    async def create_purchase(
        ...
        if cached:
            ...
            if is_payku and cached.get("transaction_id"):
                try:
                    transaction_data = await self.payku_service.verify_transaction(cached["transaction_id"])
        ...
        if existing_order:
            ...
            if existing_order.payment_provider != payment_method:
                ...
                if existing_order is None:
                    logger.debug("No hay orden existente o fue invalidada. Continuando con creación de nueva orden...")
                    ...
                    if existing_order.payment_reference:
                        try:
                            # Verificar estado de la transacción en Payku (ASYNC)
                            transaction_data = await self.payku_service.verify_transaction(existing_order.payment_reference)
                            transaction_status = transaction_data.get("status", "").lower()
                    ...
                    if existing_order.payment_reference:
                        # Si tiene payment_reference, obtener el init_point de la preferencia (ASYNC)
                        try:
                            preference = await self.mercado_pago_service.get_preference_async(existing_order.payment_reference)
                            if preference:
                    ...
                    if not payment_link:
                        try:
                            ...
                            # Crear nueva preferencia (ASYNC)
                            preference = await self.mercado_pago_service.create_preference_async(
        ...
        # Si es Payku, crear transacción de pago
            ...
            try:
                ...
                # Crear transacción con Payku (ASYNC - no bloquea el event loop)
                transaction = await self.payku_service.create_transaction(
                    order_id=str(order.id),
                    email=payer_email,
                    amount=total,
                    subject=subject,
                ...
                logger.debug(f"Transacción Payku creada: {transaction.get('transaction_id')}")
        ...
        # Si es Mercado Pago, crear preferencia de pago
            ...
            try:
                ...
                # Crear preferencia con múltiples items (ASYNC - no bloquea el event loop)
                preference = await self.mercado_pago_service.create_preference_async(
                    order_id=str(order.id),
                    currency="CLP",
                    items=items,
                ...
                logger.debug(f"Preferencia MercadoPago creada: {preference.get('preference_id')}")
                ...
                if not preference.get("payment_link"):
                    logger.error(f"Preferencia creada pero sin payment_link: {preference.get('preference_id')}")
    ...
    async def process_payment_webhook(
        ...
        if notification_type == "order":
            ...
            if not external_reference:
                try:
                    order_info = await self.mercado_pago_service.verify_order_async(resource_id)
                    external_reference = order_info.get("external_reference")
                    payment_status = order_info.get("status")
                except Exception as e:
                    logger.warning(f"No se pudo obtener order {resource_id} (puede ser simulación): {e}")
                    return False
        else:
            # Para notificaciones de tipo "payment", obtener del pago
            try:
                payment_info = await self.mercado_pago_service.verify_payment_async(resource_id)
                external_reference = payment_info.get("external_reference")
                payment_status = payment_info.get("status")
            except Exception as e:
                logger.warning(f"No se pudo obtener pago {resource_id} (puede ser simulación): {e}")
        ...
        if not external_reference:
            logger.warning("Webhook sin external_reference")
            return False
        ...
        if not order:
            logger.warning(f"Orden {external_reference} no encontrada (puede ser simulación)")
            return False
                ...
                if cached_result:
                    ...
                    if cache_timestamp_str:
                        try:
                            ...
                            logger.warning(f"Error parseando timestamp del cache: {cache_parse_error}")
                            # Continuar con verificación normal
            except Exception as cache_error:
                logger.warning(f"Error leyendo cache: {cache_error}")
                # Continuar con verificación normal

            # Si llegamos aquí, no hay cache válido o expiró, consultar Payku
            try:
                logger.info(f"Verificando estado en Payku para orden {order_id}")

                # Consultar estado en Payku (ASYNC)
                transaction_data = await self.payku_service.verify_transaction(order.payment_reference)
                ...
                try:
                    ...
                    logger.warning(f"Error guardando cache: {cache_set_error}")
                    # No es crítico, continuar

                logger.debug(f"Respuesta de Payku: {transaction_data}")
    ...
    async def _send_ticket_emails(
        ...
        El PDF se genera llamando al microservicio pdfsvc y contiene una página por ticket,
        cada una con su QR code único.
        """
        import httpx
        from datetime import datetime

        if not tickets:
            logger.warning(f"No hay tickets para enviar en orden {order.id}")
            return
        ...
        if event.starts_at:
            try:
                ...
                event_date_str = f"{dias[event_datetime.weekday()]}, {event_datetime.day} de {meses[event_datetime.month - 1]} de {event_datetime.year}"

                # Agregar hora si está disponible
                if event_datetime.hour > 0 or event_datetime.minute > 0:
                    event_date_str += f" a las {event_datetime.strftime('%H:%M')} hrs"
            except Exception as e:
                logger.warning(f"Error formateando fecha del evento: {e}")

        # Preparar datos de tickets para el PDF
        tickets_data = []
        attendees_names = []
        for ticket in tickets:
            holder_name = f"{ticket.holder_first_name} {ticket.holder_last_name}".strip()
            if holder_name:
                attendees_names.append(holder_name)

            tickets_data.append({
                "ticket_id": str(ticket.id),
                "qr_signature": ticket.qr_signature,
                "holder_first_name": ticket.holder_first_name or "",
                "holder_last_name": ticket.holder_last_name or "",
        ...
        # Generar PDF con todos los tickets usando pdfsvc
        pdf_bytes = None
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    f"{pdfsvc_url}/tickets/pdf/bulk",
                    json={
                        "tickets": tickets_data,
                        "order_id": str(order.id),
                        "buyer_name": attendees_names[0] if attendees_names else None,
                    }
                )

                if response.status_code == 200:
                    pdf_bytes = response.content
                    logger.info(f"✅ PDF generado exitosamente para orden {order.id} ({len(tickets)} tickets, {len(pdf_bytes)} bytes)")
                else:
                    logger.error(f"❌ Error generando PDF: {response.status_code} - {response.text}")
        except Exception as e:
            logger.error(f"❌ Error conectando a pdfsvc: {e}", exc_info=True)

        if not pdf_bytes:
            logger.error(f"No se pudo generar PDF para orden {order.id}, no se enviará email")
            return

        # Determinar el email del comprador principal
        # Prioridad: primer attendee con email, o el holder del primer ticket
        buyer_email = None
        buyer_name = "Estimado/a"

        for ticket in tickets:
            if ticket.holder_email:
                buyer_email = ticket.holder_email.lower().strip()
                buyer_name = f"{ticket.holder_first_name} {ticket.holder_last_name}".strip() or buyer_name
                break

        if not buyer_email:
            logger.warning(f"No hay email de comprador para orden {order.id}")
            return

        # Enviar UN solo email con el PDF adjunto
        email_service = EmailService()

        try:
            success = await email_service.send_order_tickets_email(
                ...
                attendees_names=attendees_names if len(attendees_names) > 1 else None
            )

            if success:
                logger.info(f"✅ Email con {len(tickets)} entrada(s) enviado a {buyer_email} para orden {order.id}")
            else:
                logger.error(f"❌ Error enviando email a {buyer_email} para orden {order.id}")

        except Exception as e:
            logger.error(f"❌ Error enviando email a {buyer_email}: {e}", exc_info=True)
